...
**По умолчанию выключено.** Claude Code рисует статус-строку только в своём
терминальном интерфейсе. Если работать через расширение VSCode, кэш не
пишется вообще — метры навсегда остаются на `—`, и учащение опроса файла тут
не помогает ничем.

Включённый `usage_poll` разрешает демону спрашивать тот же эндпоинт, которым
пользуется сам CLI (`/api/oauth/usage`), его же OAuth-токеном из
...
Это единственное место в мосте, которое трогает учётные данные, — потому и
выключено по умолчанию. Токен не уходит никуда, кроме Anthropic, то есть туда
же, куда его отправляет сам CLI. Эндпоинт недокументированный: если он
