...
- **Метри лімітів, по секції на вендора**: під заголовком **Claude** —
  5-годинне вікно, тижневе і власне тижневе вікно Fable; під **Codex** — його
  тижневе. Обидві секції пишуть «Тиждень» і означають різні тижні — заради
  цього заголовки й потрібні. Відсоток і колір за завантаженням (спокійний →
  жовтий ≥70% → червоний ≥90%), час обнулення — у тултіпі. Клік по заголовку
  згортає секцію, вкладка **Fable** у заголовку Claude — лише свій рядок.
- **Немає авторизації — і про це написано**: у вендора без токена (або зі
  сплилим) заголовок стає бурштиновим із ⚠, а кожен рядок під ним пояснює це
  в тултіпі — інакше стовпчик прочерків не відрізнити від зламаного віджета.
  Сплила авторизація Claude лікується запуском Claude Code, він оновлює токен
  сам; міст в облікові дані не лізе.
- **Секція є, лише поки є вендор**: немає `~/.claude` — немає рядків Claude і
  вкладки Fable; немає Codex — немає його рядка; немає обох — блок зникає
  цілком, а не висить порожнім.
- **Ліміти Codex** запитуються в того ж ендпоінта, який CLI Codex читає
  для `/status` (`chatgpt.com/backend-api/wham/usage`), раз на хвилину, з
  токеном, який CLI зберігає в `~/.codex/auth.json` — так само, як
  `usage_poll` для Claude; вимикається тумблером `codex_poll.enabled` у
  «Функціях». Це єдине джерело, яке знає про тиждень, витрачений з телефона
  чи іншої машини. Логи, які Codex і так пише
  (`~/.codex/sessions/**/rollout-*.jsonl` та `archived_sessions/`, події
  `token_count`) — запасний шлях, і перемагає свіжіше показання. Codex віддає
  по пулу на модель плюс власний пул тарифу; у рядку — тижневе вікно тарифу,
  знайдене за довжиною вікна, а не за полем, у якому воно приїхало. Якщо вікно
  вже обнулилося, показується 0%: той тиждень скінчився, а в новому нічого не
  витрачено — інакше було б свіжіше показання.
...
- **Меню** (правий клік, шестірня або іконка в треї): Проєкти, Функції,
  Прозорість, Масштаб, Кліки крізь віджет, Авто «не за ПК», Скинути положення віджета,
  Сховати в трей, Закрити. Меню і вікна «Проєкти»/«Функції» — у тому ж
  темному стилі, зміни застосовуються одразу. Версія збірки — у тултіпі
  шестірні та в підказці іконки в треї.
- **Підказки працюють і при наскрізному кліку**: вікно, виведене з
  hit-testing, не отримує ні <Enter>, ні <Leave>, тому віджет сам стежить за
  курсором і показує ту саму підказку, коли той спиняється над рядком. Саме
  там написано час обнулення, а наскрізний клік — режим, у якому картка живе
  постійно.
...
- **Скинути положення віджета**: картка, що лишилася на від'єднаному моніторі,
  повертається сама — і на старті, і на ходу. Пункт меню — ручний вихід: він
  же повертає прозорість і наскрізний клік до типових значень. Доступний із
  трея, тому працює навіть тоді, коли по картці не клікнути або її ніде не
  видно.
- **Telegram можна вимкнути цілком**: перший рядок екрана «Telegram…» і
  перший тумблер у «Функціях» (`telegram.enabled`). Вимкнений міст нічого не
  надсилає і нічого не слухає, режим «не за ПК» згортається до «за ПК»
  (заблокованому хуку однаково нікому відповісти), а з картки зникає вся
  половина про присутність: капсула, рядок будильника і пункти меню, які без
  чату нічого не означають. Лишається монітор лімітів із шестірнею. Токен
  лишається збереженим. Свіжа інсталяція стартує вимкненою; інсталяція, де
  вже є токен і група, працює далі — відсутній перемикач означає «увімкнено,
  якщо налаштовано».
- **Масштаб**: від 70% до 120% — разом шрифти, смужки і відступи, щоб картка
  вміщалася в куток, не займаючи весь куток. Tk міряє шрифт при створенні
  віджета, тому картка перезапускає себе, щоб перемалюватися в новому
  розмірі — так само, як при зміні мови.
- **Трей**: лівий клік — сховати/показати віджет, правий — меню. На іконці —
  відсоток одного з метрів, у його ж кольорі; якого саме, обирається в меню
  («У треї показувати»): сесія, тиждень, Fable чи Codex.
...
Встановлена версія оновлюється сама: демон раз на кілька годин дивиться
релізи на GitHub і ставить новий у перший же момент простою — коли немає
живих сесій і підвішених очікувань. Файл, чию контрольну суму не вдалося
звірити з опублікованою, не ставиться. З джерел оновлення — це `git pull`.
