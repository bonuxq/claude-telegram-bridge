...
- **Ліміти Codex** читаються з логів, які Codex і так пише
  (`~/.codex/sessions/**/rollout-*.jsonl`, події `token_count`): без мережі,
  без токенів, і хвіст файлу перечитується лише тоді, коли файл виріс. У
  Codex віддає по пулу на модель плюс власний пул тарифу; у рядку — тижневе
  вікно тарифу, знайдене за довжиною вікна, а не за тим, у якому полі воно
  приїхало: незайманий модельний пул показує нуль і пишеться в лог так само
  часто. Якщо вікно вже обнулилося, показується «—», а не вигаданий нуль.
